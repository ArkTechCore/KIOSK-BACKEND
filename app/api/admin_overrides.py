...
@router.get("/admin/stores/{store_id}/overrides", response_model=StoreOverridesIn)
def get_overrides(
    store_id: str,
        ...
        products=[
            {
                ...
                "active": r.active,
                "priceCentsOverride": r.price_cents_override,
            }
            for r in prods
        ],
        options=[
            {
                ...
                "active": r.active,
                "deltaCentsOverride": r.delta_cents_override,
            }
            for r in opts
        ],
    )
...
def set_overrides(
    store_id: str,
    body: StoreOverridesIn,
    session: Session = Depends(db),
    admin=Depends(require_admin_token),
):
    session.query(StoreCategoryOverride).filter(
        StoreCategoryOverride.store_id == store_id
    ).delete()
    session.query(StoreProductOverride).filter(
        StoreProductOverride.store_id == store_id
    ).delete()
    session.query(StoreOptionOverride).filter(
        StoreOptionOverride.store_id == store_id
    ).delete()
    session.commit()
